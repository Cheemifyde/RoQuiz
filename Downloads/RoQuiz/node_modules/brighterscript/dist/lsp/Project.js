...
class Project {
    ...
    async getFileRenameEdits(options) {
        ...
        const oldPkgPath = util_1.default.standardizePath(oldFile.pkgPath).toLowerCase();
        ...
        for (const file of Object.values(this.builder.program.files)) {
            if ((0, reflection_1.isBrsFile)(file)) {
                for (const importStatement of (_c = (_b = (_a = file.parser) === null || _a === void 0 ? void 0 : _a.references) === null || _b === void 0 ? void 0 : _b.importStatements) !== null && _c !== void 0 ? _c : []) {
                    if (!importStatement.filePathToken || !importStatement.filePath) {
                        continue;
                    }
                    const resolvedPkgPath = util_1.default.getPkgPathFromTarget(file.pkgPath, importStatement.filePath);
                    if (!resolvedPkgPath || util_1.default.standardizePath(resolvedPkgPath).toLowerCase() !== oldPkgPath) {
                        continue;
                    }
                    const newText = util_1.default.computeRenamedReferencePath(importStatement.filePath, file.pkgPath, newPkgPath);
                    if (newText === null) {
                        continue;
                    }
                    result.push({
                        uri: util_1.default.pathToUri(file.srcPath),
                ...
                for (const scriptTag of (_f = (_e = (_d = file.parser) === null || _d === void 0 ? void 0 : _d.references) === null || _e === void 0 ? void 0 : _e.scriptTagImports) !== null && _f !== void 0 ? _f : []) {
                    if (!scriptTag.filePathRange || !scriptTag.text) {
                        continue;
                    }
                    const resolvedPkgPath = util_1.default.getPkgPathFromTarget(file.pkgPath, scriptTag.text);
                    if (!resolvedPkgPath || util_1.default.standardizePath(resolvedPkgPath).toLowerCase() !== oldPkgPath) {
                        continue;
                    }
                    const newText = util_1.default.computeRenamedReferencePath(scriptTag.text, file.pkgPath, newPkgPath);
                    if (newText === null) {
                        continue;
                    }
                    result.push({
     ...
     */
    computePkgPathForNewSrcPath(newSrcPath, programOptions) {
        if (!programOptions.rootDir) {
            return undefined;
        }
        const destFromGlob = roku_deploy_1.rokuDeploy.getDestPath(newSrcPath, programOptions.files, programOptions.rootDir);
        if (destFromGlob) {
            return util_1.default.standardizePath(destFromGlob);
        }
        const rootDir = util_1.default.standardizePath(programOptions.rootDir);
        const newSrc = util_1.default.standardizePath(newSrcPath);
    ...
    async getSourceFixAllCodeActions(options) {
        await this.onIdle();
        if (this.builder.program.hasFile(options.srcPath)) {
