...
class ScopeValidator {
        ...
        file.ast.walk((0, visitors_1.createVisitor)({
            AAIndexedMemberExpression: (member) => {
                ...
                if ((0, reflection_1.isLiteralExpression)(member.key)) {
                    if (member.key.token.kind !== TokenKind_1.TokenKind.StringLiteral) {
                        this.addMultiScopeDiagnostic(Object.assign(Object.assign({ file: file }, DiagnosticMessages_1.DiagnosticMessages.computedAAKeyMustBeStringExpression()), { range: member.key.range }));
                    }
                    return;
                }
                const parts = util_1.default.getDottedGetPath(member.key);
                if (parts.length === 0) {
                    this.addMultiScopeDiagnostic(Object.assign(Object.assign({ file: file }, DiagnosticMessages_1.DiagnosticMessages.computedPropertyKeyMustBeConstantExpression()), { range: member.key.range }));
                    return;
                }
    ...
    constResolvesToString(value, enclosingNamespace, scope, visited = new Set()) {
        if ((0, reflection_1.isLiteralExpression)(value)) {
            return value.token.kind === TokenKind_1.TokenKind.StringLiteral;
        }
        const parts = util_1.default.getDottedGetPath(value);
        if (parts.length === 0) {
            return false;
        }
        const entityName = parts.map(p => p.name.text.toLowerCase()).join('.');
    ...
    detectCircularConstReferences() {
        ...
        const followConstRef = (expression, namespace, chain) => {
            ...
            for (const part of parts) {
                ...
                processedNames.push((_b = (_a = part.name) === null || _a === void 0 ? void 0 : _a.text) === null || _b === void 0 ? void 0 : _b.toLowerCase());
        ...
        const walkConst = (constStatement, file, chain) => {
            ...
            if (cycleStart >= 0) {
                ...
                diagnostics.push(Object.assign(Object.assign({}, DiagnosticMessages_1.DiagnosticMessages.circularReferenceDetected(items, scope.name)), { file: file, range: constStatement.tokens.name.range }));
                return;
            }
