...
class BrsFileValidator {
    ...
    walk() {
        const visitor = (0, visitors_1.createVisitor)({
            ...
            DottedGetExpression: (node) => {
                var _a;
                if (((_a = node.dot) === null || _a === void 0 ? void 0 : _a.kind) === TokenKind_1.TokenKind.QuestionDot) {
                    this.validateMinFirmwareVersionForOptionalChaining(node.dot.range);
                }
            },
            IndexedGetExpression: (node) => {
                var _a, _b, _c, _d;
                if (node.questionDotToken || ((_a = node.openingSquare) === null || _a === void 0 ? void 0 : _a.kind) === TokenKind_1.TokenKind.QuestionLeftSquare) {
                    const range = (_c = (_b = node.questionDotToken) === null || _b === void 0 ? void 0 : _b.range) !== null && _c !== void 0 ? _c : (_d = node.openingSquare) === null || _d === void 0 ? void 0 : _d.range;
                    this.validateMinFirmwareVersionForOptionalChaining(range);
                }
            },
            CallExpression: (node) => {
                var _a;
                if (((_a = node.openingParen) === null || _a === void 0 ? void 0 : _a.kind) === TokenKind_1.TokenKind.QuestionLeftParen) {
            ...
            VariableExpression: (node) => {
                ...
                const name = (_a = node.name) === null || _a === void 0 ? void 0 : _a.text;
                if (name &&
                    ...
                    this.event.file.addDiagnostic(Object.assign(Object.assign({}, DiagnosticMessages_1.DiagnosticMessages.reservedBuiltinUsedAsValue(name)), { range: node.name.range }));
                }
            }
        });
    ...
    validateMinFirmwareVersionForOptionalChaining(range) {
        ...
        if (semver.lt(minFirmwareVersion, RokuConstants_1.OPTIONAL_CHAINING_MIN_FIRMWARE_VERSION)) {
            this.event.file.addDiagnostic(Object.assign(Object.assign({}, DiagnosticMessages_1.DiagnosticMessages.featureRequiresMinFirmwareVersion('optional chaining', RokuConstants_1.OPTIONAL_CHAINING_MIN_FIRMWARE_VERSION, minFirmwareVersion)), { range: range }));
        }
    }
    /**
     ...
     */
    validateGlobalCallableAvailability(node) {
        var _a;
        if (!(0, reflection_1.isVariableExpression)(node.callee)) {
            return;
        }
        const calleeName = (_a = node.callee.name) === null || _a === void 0 ? void 0 : _a.text;
        if (!calleeName) {
            return;
        }
        const callable = globalCallables_1.globalCallableMap.get(calleeName.toLowerCase());
        const availability = callable === null || callable === void 0 ? void 0 : callable.availability;
        if (!availability) {
            return;
        }
        ...
        const diagnostic = rsgDiagnostic !== null && rsgDiagnostic !== void 0 ? rsgDiagnostic : this.computeAvailabilityDiagnostic(calleeName, 'os', availability.os, this.event.file.program.getMinFirmwareVersion());
        if (diagnostic) {
            this.event.file.addDiagnostic(Object.assign(Object.assign({}, diagnostic), { range: node.callee.range }));
        }
    }
    /**
     ...
     */
    computeAvailabilityDiagnostic(calleeName, axis, info, effectiveVersion) {
        if (!info) {
            return undefined;
        }
        if (info.removed && semver.gte(effectiveVersion, info.removed)) {
            return DiagnosticMessages_1.DiagnosticMessages.globalCallableRemoved(calleeName, axis, info.removed, effectiveVersion);
        }
        if (info.deprecated && semver.gte(effectiveVersion, info.deprecated)) {
            return DiagnosticMessages_1.DiagnosticMessages.globalCallableDeprecated(calleeName, axis, info.deprecated, effectiveVersion);
        }
        return undefined;
    }
}
