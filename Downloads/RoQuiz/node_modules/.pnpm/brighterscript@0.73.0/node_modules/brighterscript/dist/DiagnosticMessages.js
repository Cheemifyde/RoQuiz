...
exports.DiagnosticMessages = {
     ...
     */
    globalCallableRemoved: (name = '', axis = 'os', threshold = '', current = '') => ({
     ...
     * be migrated.
     */
    globalCallableDeprecated: (name = '', axis = 'os', threshold = '', current = '') => ({
        message: `'${name}' is deprecated as of ${formatAvailabilityAxis(axis, threshold)} (current target is ${current})`,
        code: 1150,
        data: { name: name, axis: axis, threshold: threshold, current: current },
        severity: vscode_languageserver_1.DiagnosticSeverity.Warning
    }),
    rsgVersionRequiresMinFirmware: (rsgVersion, requiredFirmware, configuredFirmware) => ({
