...
class RokuDeploy {
    constructor() {
        ...
        this.screenshotDir = path.join(util_1.util.tempDir, '/roku-deploy/screenshots/');
    ...
    async prepublishToStaging(options) {
        ...
        (_a = options.resolveFilesArray) !== null && _a !== void 0 ? _a : (options.resolveFilesArray = true);
        if (!options.rootDir) {
     ...
     * @param files
     */
    ensureFilesArrayIsResolved(files) {
     ...
     * (e.g. `!../../X` -> `X`).
     */
    standardizeSrcPattern(pattern) {
        const isNegated = pattern.startsWith('!');
        const stripped = isNegated ? pattern.slice(1) : pattern;
        const normalized = util_1.util.standardizePath(stripped);
    ...
    async createPackage(options, beforeZipCallback) {
        ...
        if (options.incrementBuildNumber) {
            let timestamp = (0, dateUtils_1.formatTimestampForPackage)();
    ...
    */
    async getFilePaths(files, rootDir, asAbsolute = false, stagingDir) {
            ...
            if (srcPaths) {
                for (let srcPath of srcPaths) {
                    ...
                    result.set(dest, {
                        src: srcPath,
                        dest: asAbsolute
                            ? util_1.util.standardizePath(path.resolve(stagingDir, dest))
                            : dest
                    });
                }
            }
     ...
     * @param stagingPath
     */
    async copyToStaging(files, stagingDir) {
        if (!stagingDir) {
        ...
        await Promise.all(files.map(async (fileObject) => {
            var _a;
            let destFilePath = util_1.util.standardizePath(path.resolve(stagingDir, (_a = fileObject.dest) !== null && _a !== void 0 ? _a : ''));
    ...
    async publish(options) {
        ...
        try {
            ...
            let requestOptions = this.generateBaseRequestOptions(route, options, Object.assign({ mysubmit: 'Replace', archive: readStream }, (options.appType ? { 'app_type': options.appType } : {})));
     ...
     * @returns
     */
    getPackagesFromResponseBody(body) {
        let jsonParseRegex = /JSON\.parse\(('.+')\);/igm;
        let jsonMatch;
        while ((jsonMatch = jsonParseRegex.exec(body))) {
            let [, jsonString] = jsonMatch;
     ...
     */
    async deleteComponentLibrary(options) {
        var _a;
        options = this.getOptions(options);
        let deleteOptions = this.generateBaseRequestOptions('plugin_install', options);
        deleteOptions.formData = {
        ...
        (_a = deleteOptions.qs) !== null && _a !== void 0 ? _a : (deleteOptions.qs = {});
        // eslint-disable-next-line camelcase
     ...
     */
    async deleteAllComponentLibraries(options) {
        const packages = await this.getInstalledPackages(options);
        for (const pkg of packages) {
            if (pkg.appType === 'dcl') {
                await this.deleteComponentLibrary(Object.assign(Object.assign({}, options), { fileName: pkg.archiveFileName }));
            }
        }
    }
    /**
    ...
    async takeScreenshot(options) {
        ...
        options.outFile = (_b = options.outFile) !== null && _b !== void 0 ? _b : `screenshot-${(0, dateUtils_1.formatTimestampForScreenshot)()}`;
        let saveFilePath;
     ...
     */
    async validateDeveloperPassword(options) {
        var _a, _b, _c;
        const username = (_a = options.username) !== null && _a !== void 0 ? _a : 'rokudev';
        const port = (_b = options.port) !== null && _b !== void 0 ? _b : 80;
        const timeout = (_c = options.timeout) !== null && _c !== void 0 ? _c : 3000;
        const url = `http://${options.host}:${port}/plugin_install`;
        let response;
        try {
            response = await (0, fetch_1.fetchWithDigest)(url, {
                method: 'HEAD',
                username: username,
    ...
    async getDeviceInfo(options) {
        ...
        try {
            ...
            if ((_e = (_d = (_c = (_b = (_a = e) === null || _a === void 0 ? void 0 : _a.results) === null || _b === void 0 ? void 0 : _b.response) === null || _c === void 0 ? void 0 : _c.headers) === null || _d === void 0 ? void 0 : _d.server) === null || _e === void 0 ? void 0 : _e.includes('Roku')) {
     ...
     */
    async getEcpNetworkAccessMode(options) {
        var _a, _b, _c, _d, _e;
        try {
    ...
    enhanceDeviceInfo(deviceInfo) {
        ...
        for (let key in deviceInfo) {
            result[util_1.util.camelCase(key)] = this.normalizeDeviceInfoFieldValue(deviceInfo[key]);
        }
        return result;
    }
    /**
    ...
    async zipFolder(srcFolder, zipFilePath, preFileZipCallback, files = ['**/*']) {
        ...
        for (const file of filePaths) {
            const promise = this.fsExtra.readFile(file.src).then((data) => {
                ...
                zip.file(path.relative(srcFolder, path.resolve(srcFolder, file.dest)).replace(/[\\/]/g, '/'), data, {
    ...
    async rebootDevice(options) {
        ...
        return this.doPostRequest(Object.assign(Object.assign({}, this.generateBaseRequestOptions('plugin_swup', options)), { formData: {
                mysubmit: 'Reboot',
                archive: ''
            } }));
    }
