...
const FixAllCodeActionsProcessor_1 = require("./codeActions/FixAllCodeActionsProcessor");
...
const SelectionRangesProcessor_1 = require("./selectionRanges/SelectionRangesProcessor");
const InlayHintProcessor_1 = require("./inlayHints/InlayHintProcessor");
class BscPlugin {
    constructor() {
        this.name = 'BscPlugin';
    ...
    onGetSourceFixAllCodeActions(event) {
        new FixAllCodeActionsProcessor_1.FixAllCodeActionsProcessor(event).process();
    }
    provideHover(event) {
    ...
    provideSelectionRanges(event) {
        new SelectionRangesProcessor_1.SelectionRangesProcessor(event).process();
    }
    provideInlayHints(event) {
        new InlayHintProcessor_1.InlayHintProcessor(event).process();
    }
