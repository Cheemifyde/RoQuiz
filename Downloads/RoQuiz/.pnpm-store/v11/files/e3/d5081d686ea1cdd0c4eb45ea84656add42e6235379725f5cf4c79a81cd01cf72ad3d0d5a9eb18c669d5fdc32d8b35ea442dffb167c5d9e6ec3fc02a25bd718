...
class Parser {
    ...
    addPropertyHints(item) {
        if ((0, Token_1.isToken)(item)) {
            ...
            for (const member of item.elements) {
                if (!(0, reflection_1.isCommentStatement)(member) && (0, reflection_1.isAAMemberExpression)(member) && member.keyToken) {
    ...
    whileStatement() {
        ...
        if (whileBlock && this.peek().kind === TokenKind_1.TokenKind.EndWhile) {
            ...
            this.diagnostics.push(Object.assign(Object.assign({}, DiagnosticMessages_1.DiagnosticMessages.mismatchedEndingToken(['end while'], 'next')), { range: this.peek().range }));
            endWhile = this.advance();
        }
        else {
    ...
    forStatement() {
        ...
        if (body && this.checkAny(TokenKind_1.TokenKind.EndFor, TokenKind_1.TokenKind.Next)) {
            ...
            this.diagnostics.push(Object.assign(Object.assign({}, DiagnosticMessages_1.DiagnosticMessages.mismatchedEndingToken(['end for', 'next'], 'end while')), { range: this.peek().range }));
            endForToken = this.advance();
        }
        else {
    ...
    aaLiteral() {
        ...
        let key = () => {
            ...
            result.range = util_1.util.getRange((_a = result.keyToken) !== null && _a !== void 0 ? _a : result.leftBracket, result.colon);
            return result;
        };
        while (this.match(TokenKind_1.TokenKind.Newline)) { }
        let closingBrace;
            ...
            try {
                if (this.check(TokenKind_1.TokenKind.Comment)) {
                    ...
                    lastAAMember = k.key
                        ...
                        : new Expression_1.AAMemberExpression(k.keyToken, k.colon, expr);
        ...
        this.ast.walk((0, visitors_1.createVisitor)({
            ...
            AALiteralExpression: e => {
                ...
                for (const member of e.elements) {
                    ...
                    else if ((0, reflection_1.isAAIndexedMemberExpression)(member)) {
                        this._references.expressions.add(member.value);
